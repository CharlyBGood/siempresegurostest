...
function checkInputs() {
  ...
  Object.keys(formData).forEach((key) => {
    ...
    if (!expresiones.emailIsValid.test(emailAddress.value)) {
      setErrorFor(emailAddress, "¡El email no es válido!");
    }
    
    if (!expresiones.nameIsValid.test(nameField.value)) {
      setErrorFor(nameField, "¡El campo de nombre no debe contener números ni símbolos!");
    }
    
    if (!expresiones.telIsValid.test(tel.value)) {
      setErrorFor(tel, "¡Debes escribir un Número de Tel. válido!");
    }

    if (!expresiones.postCodValid.test(postCode.value)) {
      setErrorFor(postCode, "¡Debes escribir un Código Postal válido!");
    }
  });
}
